/*===== export metadata =====
{
  "contextId" : "Context1",
  "workspaceId" : "Approved"
}
*/
/*===== business rule definition =====
{
  "id" : "GCPAuth",
  "type" : "BusinessFunction",
  "setupGroups" : [ "GCPPublish" ],
  "name" : "GCPAuth",
  "description" : null,
  "scope" : "Global",
  "validObjectTypes" : [ ],
  "allObjectTypesValid" : true,
  "runPrivileged" : false,
  "onApprove" : "Never",
  "dependencies" : [ ]
}
*/
/*===== business rule plugin definition =====
{
  "pluginId" : "JavaScriptBusinessFunctionWithBinds",
  "binds" : [ {
    "contract" : "GatewayBinding",
    "alias" : "giep",
    "parameterClass" : "com.stibo.core.domain.impl.integrationendpoint.gateway.FrontGatewayIntegrationEndpointImpl",
    "value" : "PubSubAuth",
    "description" : null
  }, {
    "contract" : "SecretBindContract",
    "alias" : "jwt",
    "parameterClass" : "com.stibo.passwordparameter.PasswordParameter",
    "value" : "",
    "description" : null
  }, {
    "contract" : "LoggerBindContract",
    "alias" : "logger",
    "parameterClass" : "null",
    "value" : null,
    "description" : null
  } ],
  "messages" : [ ],
  "pluginType" : "Operation",
  "functionReturnType" : "java.util.Map<java.lang.String, java.lang.String>",
  "functionParameterBinds" : [ ]
}
*/
exports.operation0 = function (giep,jwt,logger) {
//Return PrivateKey
function getPrivateKeyFromPem(pem) {
	const privateKeyPEM = pem
		.replace("-----BEGIN PRIVATE KEY-----", "")
          .replace("-----END PRIVATE KEY-----", "")
          .replaceAll("\\s", "");
	const encoded = java.util.Base64.getDecoder().decode(privateKeyPEM);
	const keySpec = new java.security.spec.PKCS8EncodedKeySpec(encoded);
     const keyFactory = java.security.KeyFactory.getInstance("RSA");
     return keyFactory.generatePrivate(keySpec);
}

const jwtJson = JSON.parse(jwt)
const privateKey = jwtJson.private_key;
const clientMail = jwtJson.client_email;

const header = new java.lang.String(JSON.stringify({"alg":"RS256","typ":"JWT"}))
const header_B64 = java.util.Base64.getUrlEncoder()
                    .withoutPadding()
                    .encodeToString(header.getBytes());

const now = Math.floor(java.lang.System.currentTimeMillis() / 1000);

const payload = {
	iss: clientMail,
	scope: 'https://www.googleapis.com/auth/pubsub',
	aud: 'https://oauth2.googleapis.com/token',
	iat: now,
	exp: now+3600
}
const payload_B64 = java.util.Base64.getUrlEncoder()
                    .withoutPadding()
                    .encodeToString(new java.lang.String(JSON.stringify(payload)).getBytes());
const JWT_HEADER_PAYLOAD = new java.lang.String(header_B64 + "." + payload_B64);

const signature = java.security.Signature.getInstance("SHA256withRSA");
signature.initSign(getPrivateKeyFromPem(new java.lang.String(privateKey)));
signature.update(JWT_HEADER_PAYLOAD.getBytes(java.nio.charset.StandardCharsets.UTF_8));
const signedBytes = signature.sign();

const encodedSignature = java.util.Base64.getUrlEncoder()
	.withoutPadding()
     .encodeToString(signedBytes);
const SIGNED_JWT = new java.lang.String(JWT_HEADER_PAYLOAD + "." + encodedSignature);

const formData = new java.util.HashMap()
formData.put('grant_type', 'urn:ietf:params:oauth:grant-type:jwt-bearer');
formData.put('assertion', SIGNED_JWT)

const request = giep.post().header('Content-Type', 'application/x-www-form-urlencoded').urlEncodedBody(formData)

const response = request.invoke()

const resJson = JSON.parse(response + '')
const resMap = new java.util.HashMap()
resMap.put('Authorization', 'Bearer ' + resJson.access_token)
logger.info('Token is ' + resJson.access_token)

return resMap;
}